import { createPost } from "../api/postsApi.mjs";
...
postForm.addEventListener("submit", async (e) => {
  ...
  const imageAltText =
    document.getElementById("image-alt-text").value.trim() || null;
  const tags = document
    .getElementById("tags")
    .value.split(",")
    .map((tag) => tag.trim())
  ...
  try {
    showSpinner();
    await createPost(postTitle, postContent, tags, imageUrl, imageAltText);
